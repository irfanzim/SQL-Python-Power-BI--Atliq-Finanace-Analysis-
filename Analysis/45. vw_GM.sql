/* The purpose is to join VNSMC and VFC by using keys
market+fiscal_year */
...
Add freight_cost colcumn. where freight_cost=net_sales*(1-(freight_pct+other_cost_pct))
Add COGS column, where COGS=manufacturing_cost+(net_sales*(1-(freight_pct+other_cost_pct)))
Add GM columm, where GM=net_sales-(manufacturing_cost+(net_sales*(1-(freight_pct+other_cost_pct)))) */

Create or Replace View vw_gm as

SELECT
	f.*,
    (f.net_sales*(1-(p.freight_pct+p.other_cost_pct))) as freight_cost,
    (f.manufacturing_cost+(f.net_sales*(1-(p.freight_pct+p.other_cost_pct)))) as cogs,
    (f.net_sales-(f.manufacturing_cost+(f.net_sales*(1-(p.freight_pct+p.other_cost_pct))))) as gross_margin
From
	vw_nsmc f
INNER JOIN
	vw_fc p 
